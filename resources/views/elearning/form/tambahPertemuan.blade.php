...
<main-layout>
    <title>E-Learning | Tambah Pertemuan {{ $mapel->name_mapel }} </title>
    <x-sidebar>
        <x-validation-errors class="mb-4 bg-skyblue-300 px-3 py-2" />
        <form action="{{ route('tambah-pertemuan', $mapel->slug) }}" method="post" enctype="multipart/form-data"
            autocomplete="off">
            @csrf
            <div class="relative z-0 w-full mb-2 text-center bg-sky-400 py-3">
                <p class="text-white text-2xl font-bold hover:text-black">
                    {{ $mapel->name_mapel }} : Tambah Pertemuan
                </p>
            </div>
            <div class="">
                {{-- Pertemuan ke-? --}}
