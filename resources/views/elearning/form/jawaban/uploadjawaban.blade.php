...
<body class="font-sans antialiased p-15">
    ...
    <form action="{{ route('upload-jawaban', $tugas->slug) }}" method="post" enctype="multipart/form-data"
        autocomplete="off">
        @csrf
        <div class="relative z-0 w-full mb-4 text-center bg-sky-400 py-3">
            <p class="text-white text-2xl font-bold hover:text-black">
                {{ $tugas->name_mapel }} : Upload Jawaban Tugas {{ $tugas->tugas }}
            </p>
        </div>
        ...
        <div class="relative mt-5">
            <a href="{{ route('open-tugas', $tugas->slug) }}"
